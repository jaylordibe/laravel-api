...
- `/engineering-framework:framework-doctor` — audits this repository against the framework contract (the `ef-doctor` binary the plugin puts on `PATH`). Not to be confused with `php artisan app:check-config`, which validates *runtime* configuration.
