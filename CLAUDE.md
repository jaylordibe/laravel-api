...
- `.claude/settings.json` (committed) — the permissions floor plus the `app:format` hook, and the only layer here that actually blocks anything. The engineering-framework plugin shipped command guards until 1.0.0 and deliberately no longer does; it is methodology only. That leaves a destructive command *reserved* by the session charter — which stops and hands off — rather than denied, and a deny rule is what makes the reservation unbypassable. So the destructive `./start.sh` forms and in-container migrations are denied here, and Passport key rotation is `ask`. Every rule must match the **containerised** form (`docker exec*artisan …`): a rule written as `php artisan db:wipe*` matches nothing, because almost nothing here is run from the host. The `db:wipe`/`db:seed` and `passport:*` rules are currently host-form only and so do not cover the way those commands actually get run — a known gap, not a decision.
- `.claude/engineering-framework.json` (committed) — this repository's policy, advisory by design: canonical commands, which the validation gate prefers over anything it infers, and `risk.highRiskPaths`, which raises the ceremony a design or review gate applies to a change touching one of those paths without blocking the edit. Permission and guard keys were removed in framework 1.0.0 — a `protectedCommands` block here is now schema-invalid, not merely inert.
