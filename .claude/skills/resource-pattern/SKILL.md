...
Any money/decimal column is `Brick\Math\BigDecimal`, never float — cast with `BigDecimalCast`, parse input with `BaseRequest::bigDecimal()`, divide via `App\Utils\MathUtil::divide()`. See the `money-precision` skill.

## Tests

Every resource gets a feature test (and a unit test for non-trivial calc). See `feature-testing` for the harness.
